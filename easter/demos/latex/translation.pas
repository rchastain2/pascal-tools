...
type
  ...
  TLTM = TTranslationManager;

var
  TransMgr: TTranslationManager;
...
constructor TTranslationManager.Create(AIniFile, ALanguage: AnsiString);
begin
	SetLength(FItems, 0);
  FIniFile := AIniFile;
  FLanguage := ALanguage;
...
procedure TTranslationManager.SetIniFile(s: AnsiString);
begin
	FIniFile := s;
  Load;
end;

procedure TTranslationManager.SetLanguage(s: AnsiString);
begin
  FLanguage := s;
  Load;
end;

procedure TTranslationManager.LoadLanguage(ini: TIniFile; Section: AnsiString);
var
  sl: TStrings;
  s: AnsiString;
...
procedure TTranslationManager.LoadWithoutClear;
var
  ini: TIniFile;
begin
...
procedure TTranslationManager.Load;
begin
  Clear;
  LoadWithoutClear;
end;
...
function TTranslationManager.Put(AKeyName, ATranslated: AnsiString): TLTMEntry;
var
  e: TLTMEntry;
  i: Integer;
begin
...
function TTranslationManager.Get(AKeyName: AnsiString): TLTMEntry;
var
  e: TLTMEntry;
begin
  Result := nil;
...
function TTranslationManager.GetString(AKeyName: AnsiString; DefaultValue: AnsiString = ''): AnsiString;
var
  e: TLTMEntry;
begin
	e := Get(AKeyName);
...
initialization
	TransMgr := TTranslationManager.Create;
